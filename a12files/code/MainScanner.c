...
joe_long errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern joe_long line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(joe_void);
 ...
 * -------------------------------------------------------------
 */
joe_void printScannerError(str fmt, ...);
joe_void displayScanner(BufferPointer ptrBuffer);
joe_long getScannerFilesize(str fname);
...
joe_long mainScanner(joe_long argc, str* argv) {
	...
	joe_long loadSize = 0;			/* The size of the file loaded in the buffer */
